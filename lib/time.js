...
const GRAPH_UPDATE_TIME_GAP = 30 * 1000 // 30 seconds
...
class TimeTracker {
    ...
    getServerGraphPoints = () => this._serverGraphPoints.map(TimeTracker.toSeconds)
    getGraphPoints = () => this._graphPoints.map(TimeTracker.toSeconds)
    static toSeconds = timestamp => Math.floor(timestamp / 1000)
    static getEpochMillis = () => new Date().getTime()
