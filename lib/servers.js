...
class ServerRegistration {
    ...
    constructor(app, serverId, data) {
        ...
        /** @type { { name: string, ip: string, type: 'PC' | 'PE', color: string } } */
        this.data = data
        this._pingHistory = []
    ...
    handlePing(timestamp, resp, err, version, updateHistoryGraph) {
        ...
        if (updateHistoryGraph) TimeTracker.pushAndShift(this.graphData, unsafePlayerCount, TimeTracker.getMaxGraphDataLength())
    ...
    getUpdate(timestamp, resp, err, version) {
        ...
        if (resp) {
            if (resp.version && this.updateProtocolVersionCompat(resp.version, version.protocolId, version.protocolIndex)) update.versions = this.versions
            ...
            if (this.updateFavicon(resp.favicon)) update.favicon = this.getFaviconUrl()
            if (config.logToDatabase) {
                ...
                if (this.findNewGraphPeak()) update.graphPeakData = this.getGraphPeak()
            }
        } else if (err) {
    ...
    getPingHistory() {
        if (this._pingHistory.length > 0) {
            ...
            if (graphPeakData) payload.graphPeakData = graphPeakData
    ...
    findNewGraphPeak() {
        ...
        for (let i = 0; i < this.graphData.length; i++) {
            ...
            if (point !== null && (index === -1 || point > this.graphData[index])) index = i
        }
        if (index >= 0) {
    ...
    getGraphPeak() {
        if (this._graphPeakIndex === undefined) return
        return {
    ...
    getFaviconUrl() {
        if (this.faviconHash) return Server.getHashedFaviconUrl(this.faviconHash)
    ...
    /** @returns { { name: string, ip: string, type: 'PC' | 'PE', color: string} } */
    getPublicData() {
